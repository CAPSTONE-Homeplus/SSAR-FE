import { FaDisease } from "react-icons/fa";
import { MdOutlineScubaDiving } from "react-icons/md";
import { FcManager } from "react-icons/fc";
export const services = [
  {
    title: "Lặn biển",
    icon: MdOutlineScubaDiving,
    description:
      "Trải nghiệm lặn biển cùng các chuyên gia và thế giới biển xanh bát ngát đầy màu sắc thú vị.",
    image: "https://kinhboi.com/wp-content/uploads/2022/02/lan-bien-1.jpg",
  },
  {
    title: "Du lịch sinh thái",
    icon: FaDisease,
    description:
      "Khám phá các khu bảo tồn biển và trải nghiệm những điều thú vị sẽ đến với bạn tại vùng đất này.",
    image:
      "https://bcp.cdnchinhphu.vn/334894974524682240/2022/4/2/con-dao-1648858077344216792948.jpg",
  },
  {
    title: "Hướng dẫn viên",
    icon: FcManager,
    description:
      "Được hướng dẫn bởi các chuyên gia giàu nhiệt huyết, tận tình và giàu kinh nghiệm .",
    image:
      "https://m.baotuyenquang.com.vn/media/images/2022/03/img_20220331093237.jpg",
  },
];

export const homepageList = [
  {
    title: "Các Dịch Vụ Nổi Bật",
    icon: MdOutlineScubaDiving,
    description:
      "Trải nghiệm các loại mẫu dịch vụ đa dạng và đầy thú vị.",
    image:
      "https://i.pinimg.com/736x/31/c8/e4/31c8e44217971fa28a82c0a71be7245e.jpg",
    link: "/products",
  },
  {
    title: "Mua Hàng Theo Phong Cách Riêng",
    icon: FaDisease,
    description:
      "Khám phá các mô hình mua hàng mới lạ và phù hợp với nhu cầu của bạn.",
    image:
      "https://i.pinimg.com/736x/cd/a5/ea/cda5ea6ef22739ed0c3b1bef56d90456.jpg",
    link: "/design",
  },
  {
    title: "Dịch Vụ Shipper Nhanh Chóng và An Toàn",
    icon: FcManager,
    description:
      "Đảm bảo hàng hóa của bạn được giao đến nhanh chóng và an toàn.",
    image:
      "https://i.etsystatic.com/32479513/r/il/37406b/4413285919/il_570xN.4413285919_fflr.jpg",
    link: "/about",
  },
];

export const products = [
  {
    id: "1",
    name: "Túi Xanh",
    price: 160000,
    description: "Túi màu xanh với thiết kế đơn giản, thích hợp cho nhiều dịp.",
    color: "Xanh",
    size: "M",
    image: "/images/blue-sky.jpg",
  },
  {
    id: "2",
    name: "Túi Đỏ",
    price: 200000,
    description: "Túi màu đỏ nổi bật, phù hợp cho những sự kiện đặc biệt.",
    color: "Đỏ",
    size: "L",
    image: "/images/blue-sky.jpg",
  },
  {
    id: "3",
    name: "Túi Vàng",
    price: 180000,
    description: "Túi màu vàng sáng, thời trang và cá tính.",
    color: "Vàng",
    size: "S",
    image: "/images/blue-sky.jpg",
  },
  {
    id: "4",
    name: "Túi Vàng 2",
    price: 180000,
    description: "Túi màu vàng sáng, thời trang và cá tính.",
    color: "Vàng",
    size: "S",
    image: "/images/blue-sky.jpg",
  },
  {
    id: "5",
    name: "Túi Xanh",
    price: 160000,
    description: "Túi màu xanh với thiết kế đơn giản, thích hợp cho nhiều dịp.",
    color: "Xanh",
    size: "M",
    image: "/images/blue-sky.jpg",
  },
  {
    id: "6",
    name: "Túi Đỏ",
    price: 200000,
    description: "Túi màu đỏ nổi bật, phù hợp cho những sự kiện đặc biệt.",
    color: "Đỏ",
    size: "L",
    image: "/images/blue-sky.jpg",
  },
  {
    id: "7",
    name: "Túi Vàng",
    price: 180000,
    description: "Túi màu vàng sáng, thời trang và cá tính.",
    color: "Vàng",
    size: "S",
    image: "/images/blue-sky.jpg",
  },
  {
    id: "8",
    name: "Túi Vàng 2",
    price: 180000,
    description: "Túi màu vàng sáng, thời trang và cá tính.",
    color: "Vàng",
    size: "S",
    image: "/images/blue-sky.jpg",
  },
];

export const dataBlogs = [
  {
    id: 1,
    name: "Đặt lịch nhanh chóng",
    description:
      "Thao tác 60 giây trên ứng dụng, có ngay người nhận việc sau 60 phút",
    author: "An-Thien",
    image:
      "/blog-images/1-1.jpg",
    createdDate: "2024-01-15",
    posts: [
      {
        id: 101,
        title: "Giá cả rõ ràng",
        content:
          "Giá dịch vụ được hiển thị rõ ràng trên ứng dụng. Bạn không phải trả thêm bất kỳ khoản chi phí nào.",
        tags: ["không rác thải", "bền vững", "mẹo sống xanh"],
        datePublished: "2024-08-10",
        image: "/blog-images/1-2.jpg",
      },
      {
        id: 102,
        title: "Đa dạng dịch vụ",
        content: "Chúng tôi cung cấp nhiều dịch vụ khác nhau như dọn dẹp nhà cửa, giặt ủi, chăm sóc cây cảnh, vệ sinh công nghiệp, vệ sinh nhà ở, vệ sinh công trình, vệ sinh sau xây dựng, vệ sinh sân vườn, vệ sinh nhà hàng, vệ sinh văn phòng, vệ sinh cửa hàng, vệ sinh trường học, vệ sinh bệnh viện, vệ sinh khách sạn, vệ sinh nhà trọ, vệ sinh chung cư, vệ sinh biệt thự, vệ sinh nhà xưởng, vệ sinh nhà máy",
        tags: [],
        datePublished: "",
        image: "/blog-images/1-3.jpg",
      },
      {
        id: 103,
        title: "An toàn tối đa",
        content: "Đội ngũ nhân viên được đào tạo chuyên nghiệp, có kinh nghiệm, nhiệt tình và thân thiện. Chúng tôi cam kết mang đến dịch vụ tốt nhất cho khách hàng.",
        tags: [],
        datePublished: "",
        image: "/blog-images/1-4.jpg",
      },
      {
        id: 104,
        title: "Phong cách thời trang linh hoạt",
        content: "Túi tote vải canvas có nhiều mẫu mã và kiểu dáng đa dạng, từ những chiếc túi trơn màu đến những mẫu in họa tiết sáng tạo. Dù bạn có phong cách thời trang đơn giản, thanh lịch hay cá tính, chắc chắn bạn sẽ tìm thấy một mẫu túi phù hợp với gu thời trang của mình. Không chỉ vậy, túi tote vải canvas dễ dàng kết hợp với nhiều trang phục khác nhau, tạo nên vẻ ngoài thời thượng và độc đáo.",
        tags: [],
        datePublished: "",
        image: "/blog-images/1-5.jpg",
      },
      {
        id: 105,
        title: "Dễ dàng cá nhân hóa",
        content: "Điểm đặc biệt của túi tote vải canvas là khả năng tùy chỉnh, sáng tạo. Bạn có thể tự do sáng tạo, thêm vào túi những hình vẽ, chữ viết hay hình thêu để thể hiện phong cách và cá tính riêng. Ngoài ra, tại KALBAN hiện nay cung cấp dịch vụ thiết kế túi theo yêu cầu, cho phép bạn tạo ra những chiếc túi tote độc nhất vô nhị.",
        tags: [],
        datePublished: "",
        image: "/blog-images/1-6.jpg",
      },
    ],
  },
  {
    id: 2,
    name: "Giá cả rõ ràng",
    description:
      "Giá dịch vụ được hiển thị rõ ràng trên ứng dụng. Bạn không phải trả thêm bất kỳ khoản chi phí nào.", author: "Bob Sun",
    image:
      "/blog-images/2-1.jpg",
    createdDate: "2024-02-20",
    posts: [
      {
        id: 201,
        title: "Thêm phụ kiện Charm",
        content:
          "Charm là những món phụ kiện nhỏ xinh có thể gắn vào khóa kéo hoặc quai túi xách. Những chiếc charm hình trái tim, ngôi sao, động vật, hoặc theo phong cách vintage đều sẽ giúp túi của bạn trở nên nổi bật hơn. Bạn có thể chọn charm có màu sắc hài hòa với túi hoặc những chiếc charm có ý nghĩa đặc biệt để cá nhân hóa phụ kiện này.",
        tags: [],
        datePublished: "2024-07-22",
        image: "/blog-images/2-1.jpg",

      },
      {
        id: 202,
        title: " Dùng khăn lụa trang trí quanh quai túi",
        content:
          "Một chiếc khăn lụa mềm mại quấn quanh quai túi không chỉ giúp chiếc túi thêm phần thời trang mà còn tạo nét thanh lịch, nữ tính. Hãy chọn những chiếc khăn có màu sắc hoặc họa tiết tương phản với túi để tạo điểm nhấn, hoặc phối màu theo tông trang phục của bạn để tạo sự đồng nhất. Cách này cũng giúp bạn linh hoạt thay đổi phong cách dễ dàng.",
        tags: [],
        datePublished: "2024-08-01",
        image: "/blog-images/2-2.webp",
      },
      {
        id: 203,
        title: "Vẽ trang trí trực tiếp lên túi",
        content:
          "Nếu bạn muốn một chiếc túi độc nhất vô nhị, hãy thử vẽ trực tiếp lên túi của mình! Chọn những bút màu vẽ chuyên dụng cho vải hoặc da và sáng tạo theo phong cách của riêng bạn. Bạn có thể vẽ hoa văn, hình ảnh yêu thích, hoặc chỉ đơn giản là các chữ cái, tên của mình. Cách này vừa thú vị, vừa giúp chiếc túi thể hiện đúng cá tính của bạn.",
        tags: [],
        datePublished: "2024-08-01",
        image: "/blog-images/2-3.jpg",
      },
      {
        id: 204,
        title: "Gắn huy hiệu lên túi",
        content:
          "Huy hiệu các loại pin có thể dễ dàng cài lên túi, giúp biến một chiếc túi đơn giản trở nên nổi bật. Bạn có thể chọn những huy hiệu có hình ảnh ngộ nghĩnh, cá tính hoặc gắn đinh tán kim loại để tạo vẻ ngoài mạnh mẽ, phong cách.",
        tags: [],
        datePublished: "2024-08-01",
        image: "/blog-images/2-4.webp",
      },
      {
        id: 205,
        title: "Thêm dây đeo mới",
        content:
          "Nếu dây đeo túi xách của bạn đã cũ hoặc không còn hợp mốt, thay thế nó bằng một dây đeo mới có thể là cách thay đổi dễ dàng mà hiệu quả. Hiện nay, có rất nhiều mẫu dây đeo bằng vải, kim loại hay kết hợp giữa da và xích, giúp túi xách của bạn trở nên hiện đại và phong cách hơn.",
        tags: [],
        datePublished: "2024-08-01",
        image: "/blog-images/2-5.webp",
      },
    ],
  },
  {
    id: 3,
    name: "Đa dạng dịch vụ",
    description:
      "Với 9 dịch vụ tiện ích, bTaskee sẵn sàng hỗ trợ mọi nhu cầu việc nhà của bạn.",
    author: "Clara Rivers",
    image: "/blog-images/3-1.jpg",
    createdDate: "2024-03-15",
    posts: [
      {
        id: 301,
        title: "Túi tote là gì? Vì sao cần có túi tote đeo chéo trong tủ đồ của mình?",
        content:
          "Túi tote, hay còn gọi là tote bag, là một loại túi xách được thiết kế với kiểu dáng đơn giản, rộng rãi và tiện lợi. Khác với những loại túi xách thời trang khác, túi tote thường có hình chữ nhật hoặc vuông, với hai quai dài để dễ dàng xách tay hoặc đeo trên vai. Chất liệu của túi tote cũng rất đa dạng, từ vải canvas, vải bố, da, cho đến chất liệu tái chế thân thiện với môi trường.",
        tags: [],
        datePublished: "2024-09-05",
        image: "/blog-images/3-1.jpg",
      },
      {
        id: 302,
        title: "Đặc điểm của túi tote đeo chéo",
        content: "Ngoài phiên bản truyền thống, túi tote đeo chéo còn được ưa chuộng bởi sự linh hoạt và tiện dụng. Túi tote đeo chéo giúp bạn thoải mái di chuyển mà không cần dùng tay để cầm túi, phù hợp cho những ngày bận rộn hoặc khi bạn cần mang theo nhiều vật dụng nhưng vẫn muốn tự do di chuyển.",
        tags: ["không nhựa", "bền vững", "thân thiện với môi trường"],
        datePublished: "2024-09-05",
        image: "/blog-images/3-2.jpg",
      },
      {
        id: 303,
        title: "Vì sao bạn cần có túi tote đeo chéo trong tủ đồ?",
        content: "Có nhiều lý do để sở hữu một chiếc túi tote đeo chéo trong tủ đồ, từ tính tiện dụng đến phong cách thời trang.",
        tags: [],
        datePublished: "2024-09-05",
        image: "/blog-images/3-3.jpg",
      },
      {
        id: 3.1,
        title: "Sự linh hoạt và tiện lợi",
        content: "Túi tote đeo chéo là lựa chọn hoàn hảo cho nhiều tình huống khác nhau trong cuộc sống hằng ngày. Bạn có thể mang chúng đi làm, đi học, đi chơi, hoặc thậm chí đi du lịch. Không chỉ giúp bạn dễ dàng mang theo nhiều đồ dùng, túi tote đeo chéo còn là sự kết hợp hoàn hảo giữa thời trang và sự thoải mái.",
        tags: [],
        datePublished: "2024-09-05",
        image: "/blog-images/3-4.jpg",
      },
      {
        id: 3.2,
        title: "Thân thiện với môi trường",
        content: "Nhiều loại túi tote được làm từ các chất liệu thân thiện với môi trường như vải bố, vải tái chế. Thay vì sử dụng túi nilon hay các loại túi xách có chất liệu không bền vững, túi tote là sự lựa chọn thân thiện hơn với môi trường. Đây là một bước nhỏ nhưng có ý nghĩa lớn trong việc giảm thiểu rác thải nhựa.",
        tags: [],
        datePublished: "2024-09-05",
        image: "/blog-images/3-5.jpg",
      },
      {
        id: 3.3,
        title: "Phong cách thời trang đa dạng",
        content: "Túi tote đeo chéo có nhiều kiểu dáng và mẫu mã đa dạng, phù hợp với nhiều phong cách thời trang. Từ những mẫu túi tối giản cho phong cách casual đến những mẫu túi họa tiết độc đáo cho những ai yêu thích sự nổi bật, bạn có thể dễ dàng tìm được chiếc túi phù hợp với phong cách của mình. Túi tote đeo chéo còn giúp bạn trông năng động và cá tính hơn khi kết hợp với những trang phục hàng ngày.",
        tags: [],
        datePublished: "2024-09-05",
        image: "/blog-images/3-5.jpg",
      },
      {
        id: 3.4,
        title: "Đa dụng và bền bỉ",
        content: "Túi tote đeo chéo có nhiều kiểu dáng và mẫu mã đa dạng, phù hợp với nhiều phong cách thời trang. Từ những mẫu túi tối giản cho phong cách casual đến những mẫu túi họa tiết độc đáo cho những ai yêu thích sự nổi bật, bạn có thể dễ dàng tìm được chiếc túi phù hợp với phong cách của mình. Túi tote đeo chéo còn giúp bạn trông năng động và cá tính hơn khi kết hợp với những trang phục hàng ngày.",
        tags: [],
        datePublished: "2024-09-05",
        image: "/blog-images/3-5.jpg",
      },
    ],
  },
  {
    id: 4,
    name: "Pickleball là gì? Cách chơi và kỹ thuật",
    description: "Pickleball là một môn thể thao đang ngày càng phổ biến trên toàn thế giới, đặc biệt là ở Mỹ và các nước châu  u. Đây là một sự kết hợp độc đáo giữa tennis, bóng bàn và cầu lông, tạo nên một môn thể thao thú vị, dễ học và phù hợp với mọi lứa tuổi. Nếu bạn đang tìm kiếm một môn thể thao vừa có thể rèn luyện sức khỏe vừa mang tính giải trí cao, pickleball chính là lựa chọn tuyệt vời!",
    author: "David Bloom",
    image:
      "/blog-images/4-1.jpg",
    createdDate: "2024-04-10",
    posts: [
      {
        id: 401,
        title: " Picklebaqll là gì?",
        content:
          "Pickleball ra đời vào năm 1965 tại Mỹ và được sáng tạo bởi Joel Pritchard, Bill Bell và Barney McCallum. Với mong muốn tạo ra một trò chơi mới lạ cho gia đình, họ đã kết hợp những yếu tố của nhiều môn thể thao khác nhau. Pickleball sử dụng vợt nhỏ hơn vợt tennis và bóng nhẹ hơn bóng tennis, được làm từ nhựa và có lỗ. Trò chơi diễn ra trên sân nhỏ hơn sân tennis và lưới cũng thấp hơn, mang lại cảm giác dễ tiếp cận hơn.",
        tags: ["làm vườn đô thị", "cây gia vị", "bền vững"],
        datePublished: "2024-06-30",
        image: "/blog-images/4-2.jpg",
      },
      {
        id: 402,
        title: "Cách chơi Pickleball: Quy tắc cơ bản",
        content: "Số lượng người chơi: Pickleball có thể chơi đơn (1 vs 1) hoặc đôi (2 vs 2)\n.Sân chơi: Sân có kích thước 6,1 x 13,4 m, tương tự sân cầu lông đôi, và lưới được đặt ở độ cao 0,91 m.\nCách tính điểm: Mỗi trận đấu diễn ra cho đến khi một đội đạt 11 điểm và cách biệt ít nhất 2 điểm. Điểm chỉ được ghi khi đội giao bóng thắng điểm.\nGiao bóng: Giao bóng phải thực hiện bằng cách đánh bóng dưới tay và đưa bóng qua lưới, vào vùng giao bóng đối diện.",
        tags: [],
        datePublished: "2024-06-30",
        image: "/blog-images/4-3.jpg",
      },
      {
        id: 403,
        title: "Luật chơi",
        content: "Luật không vô lưới (Non-volley): Vùng gần lưới, thường được gọi là 'kitchen' hoặc vùng 'no-volley', có chiều dài 2,1 m. Người chơi không được phép đánh bóng khi đứng trong vùng này, trừ khi bóng đã chạm đất. Luật Double-Bounce: Mỗi đội phải để bóng nảy ít nhất một lần trước khi có thể đánh trực tiếp (volley). Điều này có nghĩa là bóng phải nảy trên sân của đội nhận bóng và sau đó nảy lại một lần nữa trước khi đội giao bóng có thể volley.",
        tags: [],
        datePublished: "2024-06-30",
        image: "/blog-images/4-4.jpg",
      },
      {
        id: 404,
        title: "Các kỹ thuật cơ bản trong Pickleball",
        content: "Kỹ thuật giao bóng: Giao bóng cơ bản: Giao bóng nên bắt đầu từ phía dưới với một cú đánh lên từ dưới tay, đưa bóng qua lưới. Hãy nhắm vào vùng giao bóng đối diện để đối thủ phải di chuyển nhiều hơn. Giao bóng mạnh: Để tạo áp lực lên đối thủ, bạn có thể thực hiện cú giao bóng mạnh, buộc họ phản ứng nhanh. Tuy nhiên, điều này đòi hỏi bạn có sự chính xác cao để tránh bóng đi ra ngoài biên.",
        tags: [],
        datePublished: "2024-06-30",
        image: "/blog-images/4-5.jpg",
      },
      {
        id: 405,
        title: "Kỹ thuật điều bóng",
        content: "Dink Shot: Đây là cú đánh nhẹ, điều bóng vừa qua lưới và rơi gần vùng 'kitchen' của đối thủ. Kỹ thuật này giúp bạn khống chế trận đấu, buộc đối thủ không thể thực hiện cú volley mạnh. Groundstroke: Đây là cú đánh từ cuối sân, giúp bạn giữ khoảng cách và kiểm soát bóng từ xa. Cú đánh này yêu cầu bạn phải có lực tay và khả năng điều hướng bóng tốt. Volley: Đánh bóng trực tiếp khi chưa chạm đất, thường là cú đánh gần lưới. Để thực hiện volley thành công, bạn cần giữ tay chắc chắn và phản ứng nhanh với bóng.",
        tags: [],
        datePublished: "2024-06-30",
        image: "/blog-images/4-6.jpg",
      },
    ],
  },
  // {
  //   id: 5,
  //   name: "Khám Phá Các Chất Liệu Túi Tote Thân Thiện Với Môi Trường Cho Doanh Nghiệp Của Bạn",
  //   description:
  //     "Các loại túi tote thân thiện với môi trường không chỉ tốt cho hành tinh mà còn giúp xây dựng thương hiệu của bạn với cam kết về tính bền vững. Hãy cùng khám phá các loại chất liệu bền vững và cách chúng có thể làm nổi bật doanh nghiệp của bạn...",
  //   author: "Eva Carter",
  //   image:
  //     "https://enviro-tote.com/wp-content/uploads/2024/05/Enviro-Tote-Blog_Eco-Friendly-Tote-Bag-Materials-for-Your-Business-600x338.png",
  //   createdDate: "2024-05-20",
  //   posts: [
  //     {
  //       id: 106,
  //       title: "Lợi Ích Của Vải Tái Chế",
  //       content:
  //         "Sử dụng vải tái chế giúp giảm thiểu lượng rác thải và bảo vệ môi trường. Dưới đây là những lợi ích quan trọng của việc chuyển sang sử dụng vải tái chế...",
  //       tags: ["vải tái chế", "bền vững", "thân thiện với môi trường"],
  //       datePublished: "2024-07-15",
  //       image: "/images/blue-sky.jpg",
  //     },
  //   ],
  // },
  // {
  //   id: 6,
  //   name: "Những Chiếc Túi Đa Năng Cho Rượu Vang Làm Được Tất Cả",
  //   description:
  //     "Khách hàng của bạn đang tìm kiếm một giải pháp vừa phong cách vừa tiện dụng để mang rượu vang? Túi của chúng tôi không chỉ để đựng rượu mà còn có thể chứa sách, đồ tạp hóa hoặc bất cứ thứ gì bạn muốn! Trong những ngày bận rộn tại cửa hàng tạp hóa, túi rượu của chúng tôi mang lại một giải pháp thân thiện với môi trường thay thế túi nhựa dùng một lần. Chúng tôi có nhắc đến rằng những chiếc túi này [...]",
  //   author: "Frank Tech",
  //   image:
  //     "https://enviro-tote.com/wp-content/uploads/2024/04/Enviro-Tote_The-Multifunctional-Wine-Totes-That-Do-It-All-600x338.png",
  //   createdDate: "2024-06-01",
  //   posts: [
  //     {
  //       id: 107,
  //       title: "Đánh Giá: Những Bộ Sạc Năng Lượng Mặt Trời Tốt Nhất Năm 2024",
  //       content:
  //         "Chúng tôi đã thử nghiệm các bộ sạc năng lượng mặt trời tốt nhất trên thị trường. Đây là kết quả...",
  //       tags: ["công nghệ xanh", "sạc năng lượng mặt trời", "đánh giá"],
  //       datePublished: "2024-09-01",
  //       image: "/images/blue-sky.jpg",
  //     },
  //   ],
  // },
  // {
  //   id: 7,
  //   name: "Hướng Dẫn Của Bạn Về Kích Cỡ Túi Tote",
  //   description:
  //     "Túi tote đã trở nên cực kỳ phổ biến trong thế giới hiện đại, phục vụ như là những phương tiện vận chuyển đa năng và bền vững cho nhiều loại vật dụng. Nhưng túi tote thực sự dùng để làm gì? Từ những vật dụng hàng ngày đến nhu cầu chuyên dụng, túi tote cung cấp một giải pháp tiện lợi và phong cách để vận chuyển đồ đạc. Đây là hướng dẫn của bạn về kích cỡ túi tote và […]",
  //   author: "Grace Earth",
  //   image:
  //     "https://enviro-tote.com/wp-content/uploads/2024/03/Blog_Your-Guide-to-Tote-Bag-Sizes-600x338.png",
  //   createdDate: "2024-07-20",
  //   posts: [
  //     {
  //       id: 108,
  //       title: "Cách Bắt Đầu Chế Độ Ăn Chay",
  //       content:
  //         "Chuyển sang chế độ ăn chay dễ dàng hơn bạn nghĩ. Dưới đây là một số bước đơn giản...",
  //       tags: ["ăn chay", "sức khỏe", "thân thiện môi trường"],
  //       datePublished: "2024-07-18",
  //       image: "/images/blue-sky.jpg",
  //     },
  //   ],
  // },
  // {
  //   id: 8,
  //   name: "Lớn Hơn, Tốt Hơn: Quà Tặng Túi Tote Mà Người Tham Gia Yêu Thích",
  //   description:
  //     "Các hội chợ, sự kiện kết nối, hội nghị... đều đầy thông tin. Nhưng hãy thừa nhận, những món quà lưu niệm thực sự là các quà tặng. Người tham gia khao khát những món đồ dùng được và có thể tái sử dụng, và không có gì tuyệt vời hơn việc đảm bảo họ nhớ đến bạn qua một chiếc túi tote quảng cáo tiện lợi. Tại sao lại chọn túi tote lớn cho quà tặng của bạn? Chắc chắn, lớn hơn là tốt hơn […]",
  //   author: "Harry Traveler",
  //   image:
  //     "https://enviro-tote.com/wp-content/uploads/2024/02/Enviro-Tote-Blog_Tote-Bag-Giveaways-Attendees-Will-Love-600x338.png",
  //   createdDate: "2024-08-05",
  //   posts: [
  //     {
  //       id: 109,
  //       title:
  //         "Mẹo Du Lịch Thân Thiện Với Môi Trường Cho Chuyến Phiêu Lưu Sắp Tới",
  //       content:
  //         "Du lịch không nhất thiết phải gây lãng phí. Dưới đây là một số mẹo để du lịch bền vững...",
  //       tags: ["du lịch", "không rác thải", "du lịch bền vững"],
  //       datePublished: "2024-09-12",
  //       image: "/images/blue-sky.jpg",
  //     },
  //   ],
  // },
  // {
  //   id: 9,
  //   name: "Túi Tote Hàng Ngày: Người Bạn Đồng Hành Đáng Tin Cậy Cho Mọi Tình Huống",
  //   description:
  //     "Xin chào túi Tote Hàng Ngày, người bạn vải bố đáng tin cậy cho mọi cuộc phiêu lưu, dù lớn hay nhỏ. Đây không chỉ là chiếc túi tote thông thường; nó là công cụ mạnh mẽ nhất, sẵn sàng đối mặt với cuộc sống hàng ngày của bạn với cả phong cách và chất lượng. Được sản xuất tại Mỹ với tình yêu và chất lượng mà bạn có thể cảm nhận, Túi Tote Hàng Ngày không chỉ là một […]",
  //   author: "Ivy Innovate",
  //   image:
  //     "https://enviro-tote.com/wp-content/uploads/2023/12/The-Everyday-Tote-Your-Go-To-Companion-for-Whatever-Life-Throws-Your-Way-600x338.png",
  //   createdDate: "2024-09-10",
  //   posts: [
  //     {
  //       id: 110,
  //       title: "Gặp Gỡ Những Người Đang Cách Mạng Hóa Năng Lượng Xanh",
  //       content:
  //         "Những người tiên phong này đang dẫn đầu trong việc phát triển các giải pháp năng lượng bền vững...",
  //       tags: ["năng lượng xanh", "sáng tạo", "bền vững"],
  //       datePublished: "2024-08-25",
  //       image: "/images/blue-sky.jpg",
  //     },
  //   ],
  // },
  // {
  //   id: 10,
  //   name: "Nổi Bật Thương Hiệu Của Bạn Với Túi Tote Đeo Vai Bán Chạy Nhất Của Enviro-Tote: Chất Lượng Và Giá Trị Sản Xuất Tại Mỹ",
  //   description:
  //     "Hãy tạm biệt những chiếc túi nhựa mỏng manh, túi đeo vai đã xuất hiện! Chiếc túi vải bố phong cách này, được sản xuất ngay tại Hoa Kỳ, không chỉ là phụ kiện thông thường. Nó là công cụ đắc lực của các nhà bán lẻ, là cứu cánh của người bán sách, là người hùng của các nhà tổ chức sự kiện và là vũ khí bí mật của những người mua sắm tinh tường, những người hiểu rõ chất lượng khi họ nhìn thấy nó. Từ […]",
  //   author: "Jack Maker",
  //   image:
  //     "https://enviro-tote.com/wp-content/uploads/2023/12/Enviro-Tote-Blog_Spotlight-Your-Brand-with-Enviro-Totes-Best-Selling-Shoulder-Tote-600x338.png",
  //   createdDate: "2024-09-01",
  //   posts: [
  //     {
  //       id: 111,
  //       title: "Tự Làm Thùng Ủ Phân Hữu Cơ Cho Không Gian Nhỏ",
  //       content:
  //         "Hãy học cách làm thùng ủ phân phù hợp với không gian sống nhỏ gọn của bạn...",
  //       tags: ["DIY", "ủ phân", "dự án xanh"],
  //       datePublished: "2024-07-29",
  //       image: "/images/blue-sky.jpg",
  //     },
  //   ],
  // },
];
