  ...
  return (
    ...
    >
      ...
      <p className="text-xs mt-2 font-semibold">
        Mức độ: <span className="font-normal">{task.level}</span> | Thời gian: <span className="font-normal">{task.estimatedTime}</span>
      </p>
...
const TaskColumn: React.FC<{ status: TaskStatus; tasks: Task[]; moveTask: (taskId: number, newStatus: TaskStatus) => void }> = ({ status, tasks, moveTask }) => {
  ...
  return (
    <div ref={drop as unknown as React.Ref<HTMLDivElement>} className={`w-1/3 p-4 rounded-lg shadow-md ${isOver ? "bg-gray-200" : "bg-gray-100"}`}>
